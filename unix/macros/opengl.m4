...
AC_DEFUN(AC_NGL_OPENGL,
[
AC_CHECK_GL( [AC_CHECK_GLU(have_opengl=yes)] )

if test "$have_opengl" = yes ; then
  NGL_CFLAGS="$NGL_CFLAGS $LIBGL_CFLAGS $LIBGLU_CFLAGS"
  NGL_LIBS="$NGL_LIBS $LIBGL_LIBS $LIBGLU_LIBS"

  if test "$no_x" != yes ; then
    AC_CHECK_GLX(, have_opengl=no)
    if test "$have_opengl" = yes ; then
      NGL_CFLAGS="$NGL_CFLAGS $LIBGLX_CFLAGS"
      NGL_LIBS="$NGL_LIBS $LIBGLX_LIBS"
    fi
  fi
fi
