...
void Hoe2D::PaintRect(const THoeRect* dest,unsigned long color,bool full)
{
	...
	if (full)
	{
		...
		glVertex2f( l, t);// Levı horní bod
		glVertex2f( r, t);// Pravı horní bod
		glVertex2f( r, b);// Pravı dolní bod
		glVertex2f( l, b);// Levı dolní bod
...
void Hoe2D::PaintLine(vfloat x1,vfloat y1,vfloat x2,vfloat y2,unsigned long c)
{
	...
	glBegin(GL_LINES);
	glVertex2d( x1, y1);// Levı horní bod
	glVertex2d( x2, y2);// Pravı dolní bod
	glEnd();
...
void Hoe2D::Blt(IHoePicture * pic,const THoeRect * dest,const THoeRect * src)
{
	...
	glBegin(GL_QUADS);// Zaèátek kreslení obdélníkù
	glTexCoord2f(src->left , src->top );glVertex2f( dest->left , dest->top );// Levı horní bod
	glTexCoord2f(src->right, src->top);glVertex2f( dest->right , dest->top);// Pravı horní bod
	glTexCoord2f(src->right , src->bottom);glVertex2f( dest->right, dest->bottom );// Pravı dolní bod
	glTexCoord2f(src->left, src->bottom);glVertex2f( dest->left, dest->bottom);// Levı dolní bod
	glEnd();// Konec kreslení obdélníkù
#endif // _HOE_OPENGL_
#ifdef _HOE_D3D_
